#!/usr/bin/env python3
## python robustmpc_server.py --verbose --debug --port 8390
...
def load_movie_json(path: str, debug: bool = False) -> Dict:
    ...
    # segment_sizes_bits is [segments][qualities] in bits -> convert to bytes (ceil)
    ...
    return {
        ...
        "seg_bytes_quality": seg_bytes_quality,   # [seg][q] -> bytes
        "total_video_chunks": total_video_chunks, # max valid index
    }
...
def make_handler(
    ...
    CHUNK_TIL_VIDEO_END_CAP = float(max(TOTAL_VIDEO_CHUNKS, 1))
    ...
    def get_chunk_size(quality: int, index: int) -> int:
        ...
        if quality < 0 or quality >= len(VIDEO_BIT_RATE):
            dprint(f"[WARN] get_chunk_size bad quality: q={quality}, nQ={len(VIDEO_BIT_RATE)}")
            return 0
    ...
    class RequestHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            body = f"console.log('robust_mpc_server: ok ({movie['movie_id']})');\n"
        ...
        def do_POST(self):
            ...
            try:
                ...
                if fetch_time > 0:
                    obs[3, -1] = (chunk_size / fetch_time) / M_IN_K  # KB/ms
                else:
                    obs[3, -1] = 0.0
                    dprint(f"[WARN] rid={rid} fetch_time<=0 ({fetch_time})")

                obs[4, -1] = min(video_chunk_remain, CHUNK_TIL_VIDEO_END_CAP) / CHUNK_TIL_VIDEO_END_CAP
                measured_bw = float(obs[3, -1])

                # ---- ROBUST MPC addition: track prediction error history ----
                curr_error = 0.0
                if measured_bw > 0 and state["past_bandwidth_ests"]:
                    prev_est = float(state["past_bandwidth_ests"][-1])
                    curr_error = abs(prev_est - measured_bw) / measured_bw
                state["past_errors"].append(curr_error)

            except ZeroDivisionError:
                obs = np.zeros((S_INFO, S_LEN), dtype=np.float32) if not s_batch else np.array(s_batch[-1], copy=True)
                state["past_errors"].append(0.0)
            ...
            # Harmonic mean bandwidth over last up to 5 non-zero samples
            past_bw = obs[3, -5:]
            ...
            if len(past_bw) == 0:
                ...
                harmonic_bw = 1.0 / (sum(inv) / len(inv))

            # ---- ROBUST MPC difference: conservative future bandwidth ----
            if harmonic_bw <= 0:
                future_bw = 0.0
                max_error = 0.0
            else:
                recent_errors = state["past_errors"][-5:] if state["past_errors"] else [0.0]
                max_error = float(max(recent_errors))
                future_bw = harmonic_bw / (1.0 + max_error)
                state["past_bandwidth_ests"].append(harmonic_bw)
            ...
            dprint(
                ...
                f"sz={int(chunk_size)}B bw_meas={measured_bw:.3f} "
                f"harmonic={harmonic_bw:.3f} maxErr={max_error:.3f} robustBw={future_bw:.3f} "
                f"lookahead={future_chunk_length} reward={reward:.3f}"
            )
            vprint(f"[POST][RAW] rid={rid} keys={list(post_data.keys())}")

            # MPC search (same combinational optimizer as fastMPC)
            max_reward = -1e18
            ...
            if future_bw <= 0 or future_chunk_length == 0:
                best_combo = (0,)
                dprint(f"[ROBUST_MPC] rid={rid} fallback -> q=0 (future_bw={future_bw:.3f}, lookahead={future_chunk_length})")
            else:
                t0 = time.time()
                for full_combo in chunk_combo_options:
                    ...
                    for pos, q in enumerate(combo):
                        ...
                        dl_time = (seg_bytes / 1_000_000.0) / future_bw  # MB / (MB/s) => seconds
                ...
                t1 = time.time()
                dprint(
                    f"[ROBUST_MPC] rid={rid} chose q={best_combo[0] if best_combo else 0} "
...
def run(
    ...
    log_file_path = f"{LOG_FILE}{log_prefix}_ROBUSTMPC_{movie['movie_id']}"
    bw_file_path = f"{LOG_BW}{log_prefix}_ROBUSTMPC_{movie['movie_id']}"
    ...
    with open(log_file_path, "w", encoding="utf-8") as log_file, open(bw_file_path, "w", encoding="utf-8") as bw_file:
        ...
        state = {
            "log_file": log_file,
            "bw_file": bw_file,
            "last_bit_rate": movie["video_bit_rate_kbps"][DEFAULT_QUALITY],  # fix: bitrate, not quality index
        ...
        print('Started robustMPC server...')
