...
.. [Raw Dataset] Norway mobile bandwidth trace dataset page: ``https://skulddata.cs.umass.edu/traces/mmsys/2013/pathbandwidth/``
...
- **DQN**:   Deep Q-Network is an off-policy value-based algorithm for discrete action spaces. In ABR, it fits naturally when each action directly selects a bitrate level from a finite ladder.
- **A2C**:    Advantage Actor-Critic is a synchronous actor-critic baseline. It is simpler than PPO, but often weaker in sample efficiency and final performance.
- **PPO**   Proximal Policy Optimization is a stable on-policy actor-critic method. It is a strong ABR baseline because it usually learns steadily and avoids the brittleness of older policy-gradient approaches.
- **SAC**:   Soft Actor-Critic is an off-policy actor-critic algorithm with entropy regularization. It tends to explore well and learn robust policies, which often helps in ABR control problems with noisy network behavior.
- **DDPG**:    Deep Deterministic Policy Gradient is an off-policy continuous-control algorithm. In ABR, it can learn a continuous control signal that is later mapped back into a bitrate decision.
- **TD3** : Twin Delayed DDPG improves DDPG by reducing overestimation and stabilizing critic learning. It is frequently one of the strongest continuous-control baselines.
...
.. figure:: ../PLOT/rl_logs/walltime.png
...
The car route data includes **Oslo to Alesund**. These are long routes with both strong and very weak connectivity regions, so they stress long-horizon adaptation.
